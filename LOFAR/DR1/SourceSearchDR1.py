...
from ridge_toolkitDR1 import GetProblematicSources
...
def CreateAllPerpDisList(source_list):
    ...
    for source in source_list:
        ...
        for row in file3:
            row_info = row.strip()
            row_cols = row_info.split(',')
            xpix = float(row_cols[4])
            ypix = float(row_cols[5])
...
def CreateCutOutCat(source, lofar_table, subcat):
    ...
    np.savetxt(RLF.coc %source, cutoutcat['AllWISE','objID',str(RLF.PossRA), str(RLF.PossDEC), 'xpix', 'ypix', 'disfromLOFAR', 'raErr', 'decErr'], delimiter = ',', fmt='%s', encoding = 'utf-8')
...
def CreateDistTable(source):
    ...
    for row in file3:
        ...
        content = np.column_stack((allwise, obj_id, lofar_dis, perp_dis, distalong))        
        columns = ['AllWISE','Obj_ID', 'LOFAR_dis', 'Perp_dis', 'Dist Along']
        distances = Table(content, names = columns, dtype = ('S100', 'f8', 'f8', 'f8', 'f8'))
        
        np.savetxt(RLF.Dists %source, distances['AllWISE','Obj_ID', 'LOFAR_dis', 'Perp_dis', 'Dist Along'], delimiter = ',', fmt='%s', encoding = 'utf-8')
...
def CreatePositionTable(source_list, available_sources, lofar_table):
    ...
    #np.savetxt('Distances/Position_Info.txt', positions['Source_Name', 'AllWISE', 'LOFAR_RA', 'LOFAR_DEC', 'LOFAR_RA_errRad', 'LOFAR_DEC_errRad', 'Host_RA', 'Host_DEC', 'Host_RA_errOpt', 'Host_DEC_errOpt'], delimiter = ',', fmt='%s', encoding = 'utf-8')
    ...
    for asource in available_sources:
        if source == asource[0]:
            ...
            for row in file3:
                ...
                content = np.column_stack((allwise, sigra, sigdec, lofar_rdis, optRA, optDEC, optRAerr, optDECerr))#, perp_rdis))        
                columns = ['AllWISE', 'sigRA', 'sigDEC', 'LOFAR_Rdis', 'Opt_RA', 'Opt_DEC', 'Opt_RA_err', 'Opt_DEC_err']#, 'Perp_rdis']
                ldistances = Table(content, names = columns, dtype = ('S100', 'f8', 'f8', 'f8', 'f8', 'f8', 'f8', 'f8'))#, 'f8'))
                
                np.savetxt(RLF.LDists %source, ldistances['AllWISE','sigRA', 'sigDEC', 'LOFAR_Rdis', 'Opt_RA', 'Opt_DEC', 'Opt_RA_err', 'Opt_DEC_err'], delimiter = ',', fmt='%s', encoding = 'utf-8')
    ...
    for asource in available_sources:
        if source == asource[0]:
            ...
            for row in file3:
                ...
                content = np.column_stack((allwise, sigra, sigdec, lofar_rdis, optRA, optDEC, optRAerr, optDECerr))#, perp_rdis))        
                columns = ['AllWISE', 'sigRA', 'sigDEC', 'LOFAR_Rdis', 'Opt_RA', 'Opt_DEC', 'Opt_RA_err', 'Opt_DEC_err']#, 'Perp_rdis']
                rdistances = Table(content, names = columns, dtype = ('S100', 'f8', 'f8', 'f8', 'f8', 'f8', 'f8', 'f8'))#, 'f8'))
                
                np.savetxt(RLF.RDists %source, rdistances['AllWISE','sigRA', 'sigDEC', 'LOFAR_Rdis', 'Opt_RA', 'Opt_DEC', 'Opt_RA_err', 'Opt_DEC_err'], delimiter = ',', fmt='%s', encoding = 'utf-8')
...
def CreateSubCat(source_table, lofarra, lofardec):
    ...
    subcat = source_table[(np.abs(source_table[str(RLF.PossRA)] - lofarra) * \
                           np.cos(lofardec * np.pi / 180.0) < RLC.rsize) & \
                                (np.abs(source_table[str(RLF.PossDEC)] - lofardec) < RLC.dsize)]
...
def GetSourceList(available_sources, probfile):
    ...
    asource_names - list,
                    a list of the source ID names of the successful
                    sources. 
    ...
    psource_names = list(problematic_sources[:,0])
    asource_names = list(available_sources[:,0])

    for source in psource_names:
        asource_names.remove(source)

    return asource_names
    ...
    for source in source_list:
        for asource in available_sources:
            if source == asource[0]:
                ...
                RLr = LDistance(RLdelRA, RLdelDEC, RLsigRA, RLsigDEC)
...
def NClosestDistances(source_list, available_sources, lofar_table, n):
    ...
    for source in source_list:
        ...
        for row in file5:
            ...
            row_cols = row_info.split(',')
            allw = row_cols[0].strip("''").strip('b').strip("''")
            sRA = float(row_cols[1])
            sDEC = float(row_cols[2])
            distLR = float(row_cols[3])
            distLR /= lmsize
            optRA = float(row_cols[4])
            optDEC = float(row_cols[5])
            optRAerr = float(row_cols[6])
            optDECerr = float(row_cols[7])
        ...
        rsiganderr = np.column_stack((nsortids, nsortLRD, nsortra, nsortdec, nsortoptra, nsortoptdec, nsortraerr, nsortdecerr))
        
        columns = ['AllWISE', 'Lofar_R_Dist', 'Sigma_RA', 'Sigma_DEC', 'Opt_RA', 'Opt_DEC', 'Opt_RA_err', 'Opt_DEC_err']
        Nclosest = Table(rsiganderr, names = columns, dtype = ('S100', 'f8', 'f8', 'f8', 'f8', 'f8', 'f8', 'f8'))
        
        np.savetxt(RLF.NDist %source, Nclosest['AllWISE', 'Lofar_R_Dist','Sigma_RA', 'Sigma_DEC', 'Opt_RA', 'Opt_DEC', 'Opt_RA_err', 'Opt_DEC_err'], delimiter = ',', fmt='%s', encoding = 'utf-8')
...
def SourceInfo(source, lofar_table):
    ...
    return lofarra, lofardec, size, allwise, objid, redz
...
def TableFromLofar(lofarcat):
    ...
    (the source redshift) and LM_size (the source size in arcseconds).
    ...
    Lwise = tbdata.field(str(RLF.All))  ## Find the AllWISE ID
    LID = tbdata.field(str(RLF.OID))  ## Find the object ID of the source
    Lz = tbdata.field(str(RLF.LredZ))  ## Find the redshift of the source
    Lsize = tbdata.field(str(RLF.LMS))  ## Find the size of the source (arcsec)    
    source_names = np.column_stack((Name, Lra, Ldec, Lwise, LID, Lz, Lsize))
    ## Stack all four columns next to each other. Note: Does it deal with missing data?
    columns = [str(RLF.LSN), str(RLF.LRA), str(RLF.LDEC),str(RLF.All), str(RLF.OID), str(RLF.LredZ), str(RLF.LMS)]  ## Creates column headings for calling rather than indices
    Ltable = Table(source_names, names = columns, dtype = ('S100', 'f8', 'f8', 'S100', 'f8', 'f8', 'f8'))  ## Turns it in to a table
...
def TableOfSources(catalogue):
    ...
    ID = tbdata.field(str(RLF.IDP))  ## Find the object ID of the source 
    ...
    ra_err = tbdata.field(str(RLF.PRAErr))  ## Find the error on the RA position (arcsec)
    dec_err = tbdata.field(str(RLF.PDECErr))  ## Find the error on the DEC position (arcsec)
    w1 = tbdata.field(str(RLF.OptMagA))  ## Find the W1 magnitude
    w1_err = tbdata.field(str(RLF.MagAErr))  ## Find the error on the W1 magnitude
    i = tbdata.field(str(RLF.OptMagP))  ## Find the i magnitude
    i_err = tbdata.field(str(RLF.MagPErr))  ## Find the error on the i magnitude
    source_names = np.column_stack((WISE, ID, ra, dec, ra_err, dec_err, w1, w1_err, i, i_err))
    columns = [str(RLF.IDW), str(RLF.IDP), str(RLF.PossRA), str(RLF.PossDEC), str(RLF.PRAErr), str(RLF.PDECErr), str(RLF.OptMagA), str(RLF.MagAErr), str(RLF.OptMagP), str(RLF.MagPErr)]  ## Creates column headings for calling rather than indices
    table = Table(source_names, names = columns, dtype = ('S100', 'f8', 'f8', 'f8', 'f8', 'f8', 'f8', 'f8', 'f8', 'f8'))  ## Turns it in to a table
