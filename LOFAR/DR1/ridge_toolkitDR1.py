...
def GetAvailableSources(filename):
    ...
    for source in sources_list:
        ...
        LraE = float(info_cols[8])
        LdecE = float(info_cols[9])
        fits_name = str(RLF.fits) + source_name + '.fits'
...
#            source_idypix += 0.5
            ...
            source_summary = np.array([source_name, source_Lxpix, \
                    ...
                    source_idxpix, source_idypix, LraE, LdecE, idrx, idry])
...
def TotalFluxSelector(catalogue1):
    ...
    Names = tbdata.field('Source_Name')  ## Find the Source Name Column
    Flux = tbdata.field('Total_flux')  ## Find the Total Flux Column
    Lra = tbdata.field('RA')  ## Find the RA position Column of the LOFAR position
    LraE = tbdata.field('E_RA')  ## Find the error on the RA of the LOFAR position
    Ldec = tbdata.field('DEC')  ## Find the DEC position Column of the LOFAR position
    LdecE = tbdata.field('E_DEC')  ## Find the error on the DEC of the LOFAR position
    compnum = tbdata.field('LGZ_Assoc')  ## Find the number of associated components
    size = tbdata.field('LM_Size')  ## Find the source size in Arcsecs
    idra = tbdata.field('ID_ra')  ## Find the Optical ID RA
    iddec = tbdata.field('ID_dec')  ## Find the Optical DEC
    #MLLR = tbdata.field('ML_LR')  ## Find the value of the LR for the host of the source
    source_names = np.column_stack((Names, Flux, Lra, Ldec, compnum, size, idra, iddec, LraE, LdecE))
    ## Stack all four columns next to each other. Note: Does it deal with missing data?
    columns = ['Source_Name', 'Total_flux', 'RA', 'DEC', 'LGZ_Assoc', 'LM_Size', 'ID_ra', 'ID_dec', 'E_RA', 'E_DEC']  ## Creates column headings for calling rather than indices
    ...
    sub5 = table['LM_Size'] > 60 ## S2  THE ONE I WANT
    sub9 = table['Total_flux'] > 30 ## F4  THE ONE I WANT
    ...
    #sub10 = table['LGZ_Assoc'] == RLC.LGZA
    ...
    np.savetxt(str(RLF.TFC), output['Source_Name','RA','DEC', 'LGZ_Assoc', 'LM_Size', 'Total_flux', 'ID_ra', 'ID_dec', 'E_RA', 'E_DEC'], fmt='%s', encoding = 'utf-8')  ## Creates a text file with source id, position, number of components, size and flux in it.
...
def TrialSeries(available_sources, components, R, dphi, CompTable):
    ...
    **NOTE - Matplotlib is currently throwing up a warning about the
    ...
    for source in available_sources[RLC.Start::RLC.Step]:
        ...
        source_name = source[0]
        n_comp = source[3]
        cat_pos = source[4:6].astype('float64')
        lmsize = source[6]
